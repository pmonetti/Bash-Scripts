...
RM_FILES_PATH=$OUTPUT_DIR/files_to_remove.txt
...
get_external_path()
{
    ...
    if [[ "${OUT_DIR:0:1}" == / || "${OUT_DIR:0:2}" == ~[/a-z] ]] ; then
      # $OUT_DIR refers to an absolute dir path
      ABS_OUT_DIR=$OUT_DIR
    else
      CURRENT_DIR=${PWD}      
...
process_single_file()
{
  ...
  if [ -n "$MV_EXTENSION" ] && [ "$EXTENSION" == "$MV_EXTENSION" ] && [ -n "$MV_OUTPUT" ] ; then
    NEW_PATH=$(get_external_path "$MV_OUTPUT" "$NEW_PATH")
  fi
  
  if [ "$NEW_PATH" != "$FILEPATH" ] ; then
    echo "Final Path "$NEW_PATH
    echo $FILENAME >> $RM_FILES_PATH    
  fi
}
